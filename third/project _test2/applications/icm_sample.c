...
static int key_up_callback(void *args)
{
    ...
    if(!value)
    {
        ...
        rt_pin_write(BUZZER, 0);
    }
    
    
}   

static void KEY_example(void)
{
...
static void icm_thread_entry(void *parameter)
{
    ...
    if(abs(pitch) > 30 || abs(roll) > 30)
    {
        u8g2_DrawStr(&u8g2,40, 25, "!! Tumble");
        // u8g2_ClearBuffer(&u8g2);
    }
    else
    {
        u8g2_DrawStr(&u8g2,40, 25, "Healthy");
...
INIT_COMPONENT_EXPORT(icm_example);